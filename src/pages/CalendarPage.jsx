            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-4 bg-zinc-900/50 p-2 rounded-2xl border border-zinc-800 backdrop-blur-sm">
                        <button onClick={prevMonth} className="p-2 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-xl transition-colors">
                            <Icons.ChevronRight className="w-5 h-5 rotate-180" />
                        </button>
                        <div className="text-center min-w-[140px]">
                            <h2 className="text-lg font-bold text-white uppercase">{format(currentDate, 'MMMM yyyy')}</h2>
                        </div>
                        <button onClick={nextMonth} className="p-2 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-xl transition-colors">
                            <Icons.ChevronRight className="w-5 h-5" />
                        </button>
                        <div className="w-px h-6 bg-zinc-800 mx-2" />
                        <button onClick={jumpToToday} className="px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-zinc-400 hover:text-white border border-zinc-700 rounded-lg hover:border-zinc-500 transition-all">
                            Today
                        </button>
                    </div>
                </div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl">
                ...
                <div className="grid grid-cols-7 border-b border-zinc-800 bg-zinc-950/30">
                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, i) => (
                        <div key={day} className={clsx("py-3 text-center text-xs font-bold uppercase tracking-widest text-zinc-500", i === 0 || i === 6 ? "bg-red-500/5" : "")}>
                            {day}
                        </div>
                    ))}
                </div>
                            ...
                            >
                                ...
                                <div className="flex justify-between items-start mb-2">
                                    <span className={clsx(
                                        ...
                                        isTodayDate ? "bg-blue-500 text-white" : "text-zinc-500 group-hover:text-zinc-300"
                                    )}>
                                        {format(day, 'd')}
                                    </span>
                                    {dayEvents.length > 0 && (
                                ...
                                <div className="space-y-1 overflow-visible">
                                        ...
                                        <div
                                            ...
                                            className={clsx(
                                                ...
                                                evt.type === 'event'
                                                    ...
                                                    : evt.color === 'green'
                                                        ? "bg-emerald-500/10 text-emerald-300 border-emerald-500 hover:bg-emerald-500/20"
                                            ...
                                            <AnimatePresence>
                                                {hoveredEvent?.id === evt.id && (
                                                    <motion.div
                                                        ...
                                                        className="absolute left-0 bottom-full mb-2 w-64 p-3 bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl z-[100]"
                                                    // Allow clicks to bubble up to parent
                                                    >
                                                        ...
                                                        {/* Arrow */}
                                                        <div className="absolute left-4 bottom-0 translate-y-full border-8 border-transparent border-t-zinc-700" />
                                                    </motion.div>
                                                )}
                                            </AnimatePresence>
                                        </div>
                                    ...
                                    {dayEvents.length > 4 && (
                                        <div className="text-[10px] text-zinc-500 pl-1 font-bold">
